...
for file in files:
    ...
    # ==========4-6.提取政策范围、工具类型和执行机构=====Extract policy scope, tool types, and implementing agencies.======
    ...
    for doc in split_docs:
        # ==========6.提取政策执行机构=====Extract policy implementing agencies.======
        ...
        # ==========7.进行政策文本摘要=====Conduct a summary of policy documents.======
        prompt7 = ('对于给定的政策文本片段，帮我进行摘要，200字以内；'
                  '只回复摘要结果，不要其他的文字。文本如下：')
        #prompt7 = For a given policy text snippet, help me summarize it within 200 words. Only reply with the summary result and no other words. The text is as follows: 
        prompt7 += doc.page_content
        res = llm_chain.invoke(prompt7)
        # print(res['text'])
        extraction_temp += res['text']

    # ==========6.提取政策执行机构汇总=====Extract a summary of policy implementing agencies.======
    ...
    # ==========7.根据政策文本摘要提取政策功能和措施=====Based on the summary of the policy document, extract the policy functions and measures.======
    ...
    # ==========9.根据政策文本摘要提炼政策主要为哪些对象解决金融服务困难=====Based on the summary of the policy document, identify the main target groups for which the policy addresses difficulties in accessing financial services.======
